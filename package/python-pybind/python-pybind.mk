...
PYTHON_PYBIND_INCLUDE_PATH = $(HOST_DIR)/lib/python$(PYTHON3_VERSION_MAJOR)/site-packages/include/

define PYTHON_PYBIND_INSTALL_MODULE
	mkdir -p $(PYTHON_PYBIND_INSTALL_PATH)
	mkdir -p $(PYTHON_PYBIND_INCLUDE_PATH)
	cp -dpf $(@D)/pybind11/*.py $(PYTHON_PYBIND_INSTALL_PATH)
	cp -a $(STAGING_DIR)/usr/include/pybind11 $(PYTHON_PYBIND_INCLUDE_PATH)
endef
